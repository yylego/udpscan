...
// ScanSign is the magic string both sides must send to shake hands.
// ScanSign 是 UDP 广播扫描协议中使用的魔术字符串。
const ScanSign = "UDPSCAN"

// ScanPort is the default UDP port both sides use.
...
// MaxPacketSize is the largest payload a single IPv4 UDP datagram can hold.
// MaxPacketSize 是单个 IPv4 UDP 报文能承载的最大载荷，用作接收缓冲区大小。
//
// Anything below this size cuts oversized responses short, and a truncated response
...
// SafePayloadSize is the UDP payload that fits inside one Ethernet frame unfragmented.
// SafePayloadSize 是仍能装进一个以太网帧、不触发 IP 分片的 UDP 载荷上限。
//
// 1500 (Ethernet MTU) - 20 (IPv4) - 8 (UDP) = 1472, rounded down.
...
//
// The Username and SSHPort fields let the client compose a runnable ssh command.
// Username 和 SSHPort 字段让客户端能拼出可直接执行的 ssh 命令。
type Response struct {
	Name string    `json:"name"`
	Time time.Time `json:"time"`
	// Username is the account to log in with, not always the account running this service.
...
// IsValidPort reports if the value is a usable TCP/UDP port.
// IsValidPort 判断给定的值是不是可用的 TCP/UDP 端口号。
//
// Zero is rejected here: it means "let the system choose" in some contexts, which is
// a decision left to the invoking code, not a trait of the value itself.
...
//
// Returns nil when the address and mask are not a usable IPv4 combination, so callers can
...
// ScanResult is the complete outcome of one scan, shaped to suit JSON output.
// ScanResult 是一次扫描的完整结果，形状是为 JSON 输出设计的。
//
// The overview fields prove worthwhile when Hosts is blank: each one sets apart "no host
// responded" from "the probes left via the wrong interface".
...
// SSHCommand composes a runnable ssh command from the response and the given IP.
// SSHCommand 用响应内容和给定的 IP 拼出可直接执行的 ssh 命令，拼不出时返回空串。
//
// The IP must come from the scanning side instead of from the response, because a host
// with multiple interfaces cannot know which of its addresses did reach the client.
// IP 必须由调用方传入、而不能由服务端自报：多网卡的主机并不知道自己是通过哪个地址被看到的，
// 只有客户端手里那个"实际收到应答的地址"才保证可达。
func (resp *Response) SSHCommand(ip string) string {
	if resp.Username == "" || ip == "" {
		return ""
	}
	if resp.SSHPort != 0 && resp.SSHPort != DefaultSSHPort {
		return fmt.Sprintf("ssh -p %d %s@%s", resp.SSHPort, resp.Username, ip)
	}
	return fmt.Sprintf("ssh %s@%s", resp.Username, ip)
}
